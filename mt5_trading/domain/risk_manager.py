...
class RiskManager:
    ...
    def calculate_position_size(
        ...
        try:
            ...
            logger.info(
                f"Calculated position size for {symbol}: {position_size} lots "
    ...
    def get_total_exposure(self, symbols: list[str] | None = None) -> dict:
        ...
        try:
            ...
            return {"total_positions": 0, "total_volume": 0.0, "total_profit": 0.0, "symbols": {}}

    def check_risk_limits(
        self, symbol: str, proposed_position_size: float, stop_loss_pips: float
    ) -> tuple[bool, str]:
        """
        ...
        try:
            ...
            if position_risk_percent > (self.risk_per_symbol * 100):
                return (
                    False,
                    f"Position risk ({position_risk_percent:.2f}%) exceeds per-symbol limit ({self.risk_per_symbol * 100:.2f}%)",
                )
            ...
            if new_total_risk > (self.max_total_risk * 100):
                return (
                    False,
                    f"Total risk ({new_total_risk:.2f}%) would exceed maximum limit ({self.max_total_risk * 100:.2f}%)",
                )

            return (
                True,
                f"Risk check passed (position risk: {position_risk_percent:.2f}%, total risk: {new_total_risk:.2f}%)",
            )
    ...
    def get_volatility_multiplier(self, atr_percentage: float, base_atr: float = 1.0) -> float:
        """
