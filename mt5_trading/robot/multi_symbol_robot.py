...
class MultiSymbolRobot:
        ...
        try:
            ...
            if volatility_metrics:
                ...
                if volatility_multiplier == 1.0:
                    volatility_multiplier = self.risk_manager.get_volatility_multiplier(
                        atr_percentage
                    )
        ...
        try:
            ...
            if signal == Signal.BUY:
                ...
                if total_buy == 0:
                    ...
                    if result.retcode == mt5.TRADE_RETCODE_DONE:
                        logger.info(
                            f"Buy position opened for {symbol}: "
            ...
            # Process sell signal
                ...
                if total_sell == 0:
                    ...
                    if result.retcode == mt5.TRADE_RETCODE_DONE:
                        logger.info(
                            f"Sell position opened for {symbol}: "
                            f"Order #{result.order}, Volume: {result.volume}, Price: {result.price}"
                        )
                    else:
    ...
    def trade(self):
        ...
        logger.info(
            f"Current exposure: {exposure['total_positions']} positions, "
