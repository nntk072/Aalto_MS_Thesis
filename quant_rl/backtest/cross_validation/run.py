...
def compare_results(backtrader_result: dict[str, Any], custom_result: dict[str, Any], 
    ...
    comparison['agreement']['trade_count_diff'] = backtrader_result['trade_count'] - custom_result['trade_count']
...
def print_report(comparison: dict[str, Any]) -> None:
    ...
    if not agree['pnl_within_tolerance']:
        print(f"    Difference: ${agree['pnl_diff']:.2f} (tolerance: ${agree['pnl_tolerance']:.2f})")
    
    print(f"  Max Drawdown within tolerance: {'✓ PASS' if agree['max_drawdown_within_tolerance'] else '✗ FAIL'}")
    if not agree['max_drawdown_within_tolerance']:
        print(f"    Difference: {agree['max_drawdown_diff']:.2%} (tolerance: {agree['max_drawdown_tolerance']:.2%})")
